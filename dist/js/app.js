(() => {
				...
				for (let t in n)
					if (n.hasOwnProperty(t)) {
						...
						if (e && (Array.isArray(s) || pt(s))) {
							...
							i[t] = ut(!0, i.hasOwnProperty(t) ? i[t] : e, s);
						} else i[t] = s;
					}
			}
			return i;
	...
	class xt {
		constructor(t = {}) {
			(this.options = ut(!0, {}, t)), (this.plugins = []), (this.events = {});
		...
		attachPlugins(t) {
			...
			for (const [i, s] of Object.entries(t || {}))
				!1 === this.options[i] ||
					this.plugins[i] ||
					((this.options[i] = ut({}, s.defaults || {}, this.options[i])),
						(e[i] = new s(this)));
	...
	class St extends xt {
		constructor(t, e = {}) {
			super(ut(!0, {}, Ct, e)), (this.state = "init"), (this.$container = t);
			for (const t of ["onLoad", "onWheel", "onClick"])
				this[t] = this[t].bind(this);
			this.initLayout(),
				this.resetValues(),
				this.attachPlugins(St.Plugins),
				this.trigger("init"),
				this.updateMetrics(),
				this.attachEvents(),
				this.trigger("ready"),
		...
		updateMetrics(t) {
			...
			(c = ft(c * p)), (h = ft(h * p)), (r = c), (l = h);
			const u = i.getBoundingClientRect(),
				f = s.getBoundingClientRect(),
				m = s == e ? f : e.getBoundingClientRect();
			let g = Math.max(s.offsetWidth, ft(f.width)),
				v = Math.max(s.offsetHeight, ft(f.height)),
	...
	class kt extends xt {
		constructor(t, e = {}) {
			...
			(this.slideNext = Et(this.slideNext.bind(this), 250)),
				(this.slidePrev = Et(this.slidePrev.bind(this), 250)),
				this.init(),
				(t.__Carousel = this);
		}
		init() {
			(this.pages = []),
				...
				this.$track &&
				this.pages.length &&
				(this.$track.style.transform = `translate3d(${-1 * this.pages[this.page].left
					}px, 0px, 0) scale(1)`),
		...
		initSlides() {
			(this.slides = []),
				...
				(this.slides = ut(!0, [...this.slides], this.options.slides));
		}
		updateMetrics() {
			let t,
				e = 0,
		...
		initPanzoom() {
			...
			(this.Panzoom = new St(this.$container, t)),
				this.Panzoom.on({
	...
	(kt.version = "4.0.27"), (kt.Plugins = Pt);
	const Mt = !(
		...
		Ot = (t) => {
			if (t && Mt) {
				...
				try {
					...
					else if (At) t.focus({ preventScroll: !0 });
					else {
		...
		processType(t) {
			...
			if (!e || "string" == typeof e) {
				if (
					...
					const n = It(e, this.fancybox.option("Html.youtube")),
					...
					const n = It(e, this.fancybox.option("Html.vimeo")),
						o = encodeURIComponent(s[1]);
					(t.videoId = o),
				...
				i ||
					...
					((t.video = ut({}, this.fancybox.option("Html.video"), t.video)),
						t._width && t._height
							? (t.ratio = parseFloat(t._width) / parseFloat(t._height))
							: (t.ratio = t.ratio || t.video.ratio || Nt.video.ratio));
			}
		}
		onReady() {
	...
	class Ft {
		...
		initSlidePanzoom(t) {
			t.Panzoom ||
				((t.Panzoom = new St(
					t.$el,
					ut(!0, this.fancybox.option("Image.Panzoom", {}), {
						viewport: t.$wrap,
						content: t.$image,
						width: t._width,
						height: t._height,
	...
	class Bt {
		...
		static startFromUrl() {
			...
			const { hash: e, slug: i, index: s } = Bt.getParsedURL();
			if (!i) return;
		...
		static onHashChange() {
			const { slug: t, index: e } = Bt.getParsedURL(),
				i = Bt.Fancybox,
				s = i && i.getInstance();
			if (s && s.plugins.Hash) {
				if (t) {
		...
		static create(t) {
			function e() {
				window.addEventListener("hashchange", Bt.onHashChange, !1),
					Bt.startFromUrl();
			}
			(Bt.Fancybox = t),
				Mt &&
		...
		static destroy() {
			window.removeEventListener("hashchange", Bt.onHashChange, !1);
		}
		static getParsedURL() {
	...
	const Rt = {
		...
		activate(t) {
			(Rt.pageXOffset = window.pageXOffset),
				(Rt.pageYOffset = window.pageYOffset),
				t.requestFullscreen
					? t.requestFullscreen()
			...
			fullscreen: {
				...
				click: function (t) {
					t.preventDefault(),
						Rt.element()
							? Rt.deactivate()
							: Rt.activate(this.fancybox.$container);
				},
			},
			slideshow: {
				type: "button",
	...
	class Gt {
			...
			if (
				...
				(this.build(),
					...
					(t.option("slideshow.autoStart") && this.Slideshow.activate(),
						t.option("fullscreen.autoStart") && !Rt.element()))
			)
				try {
					Rt.activate(t.$container);
				} catch (t) { }
		}
		onFsChange() {
			window.scrollTo(Rt.pageXOffset, Rt.pageYOffset);
		}
		onSettle() {
			const t = this.fancybox,
				e = this.Slideshow;
		...
		build() {
			...
			for (const s of this.fancybox.option("Toolbar.display")) {
				let n, o;
				if (
					(pt(s) ? ((n = s.id), (o = ut({}, t[n], s))) : ((n = s), (o = t[n])),
	...
	const qt = {
		ScrollLock: class {
			...
			onWheel(t) {
				mt(t.composedPath()[0]) || t.preventDefault();
			}
			cleanup() {
				this.pendingUpdate &&
	...
	class Yt extends xt {
		...
		onClick(t) {
			...
			if (e.matches("[data-fancybox-close]"))
				return t.preventDefault(), void Yt.close(!1, t);
			if (e.matches("[data-fancybox-next]"))
				return t.preventDefault(), void Yt.next();
			if (e.matches("[data-fancybox-prev]"))
				return t.preventDefault(), void Yt.prev();
			if (
				!(e.matches(_t) || document.activeElement.blur(),
		...
		onKeydown(t) {
			if (Yt.getInstance().id !== this.id) return;
		...
		focus(t) {
			if (Yt.ignoreFocusChange) return;
			if (
			...
			t && t.preventDefault(), (Yt.ignoreFocusChange = !0);
			const n = Array.from(e.querySelectorAll(_t));
			let o,
				a = [];
			for (let t of n) {
		...
		static fromOpener(t, e = {}) {
			let i = [],
				...
				n = e.target || null;
			const o =
				void 0 !== (e = ut({}, e, Yt.openers.get(t))).groupAll && e.groupAll,
			...
			return (
				...
				((s = n ? i.indexOf(n) : s),
					...
					new Yt(i, ut({}, e, { startIndex: s, $trigger: n })))
			);
		}
		static bind(t, e = {}) {
			function i() {
				document.body.addEventListener("click", Yt.fromEvent, !1);
			}
			Mt &&
				(Yt.openers.size ||
					(/complete|interactive|loaded/.test(document.readyState)
		...
		static unbind(t) {
			Yt.openers.delete(t), Yt.openers.size || Yt.destroy();
		}
		static destroy() {
			let t;
			for (; (t = Yt.getInstance());) t.destroy();
			(Yt.openers = new Map()),
				document.body.removeEventListener("click", Yt.fromEvent, !1);
		}
		static getInstance(t) {
			return t
				? Vt.get(t)
				: Array.from(Vt.values())
					.reverse()
					.find(
						(t) =>
		...
		static close(t = !0, e) {
			if (t) for (const t of Vt.values()) t.close(e);
			else {
				const t = Yt.getInstance();
				t && t.close(e);
			}
		}
		static next() {
	...
	(Yt.version = "4.0.27"),
		...
		(Yt.openers = new Map()),
		(Yt.Plugins = qt),
		Yt.bind("[data-fancybox]");
	for (const [t, e] of Object.entries(Yt.Plugins || {}))
		"function" == typeof e.create && e.create(Yt);
